...
#	my $reference=$d{'SOURCEFOOTPRINTLIBRARY'}."/".$d{'SOURCELIBREFERENCE'};
  ...
  #HandleBinFile("$short/Root Entry/Pads6/Data.dat","\x02",0,0, sub 
  {
	...
	while($pos+140<length($value))
	{
      ...
      assertdata("Pad",$counter,"PASTEMASKEXPANSION_MANUAL",bmil2(substr($value,$pos+109,4))) if(bmil2(substr($value,$pos+109,4)) ne "0mil");
...
if(defined($stp));
  ...
  { 
    ...
    my $USERROUTED=unpack("C",substr($value,55,1)); # AUTOGENERATED
...
;
  ...
  { 
	...
	my $USERROUTED=unpack("C",substr($value,44,1)); # AUTOGENERATED
...
;
  ...
  { 
    ...
    my $USERROUTED=unpack("C",substr($value,45,1)); # AUTOGENERATED
...
#    my $olayer=unpack("C",substr($value,31,1));# AUTOGENERATED, possibly other format
#    assertdata("Region",$_[3],"LAYER",$altlayername{$olayer}) if(defined($altlayername{$olayer}));
