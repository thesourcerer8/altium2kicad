...
{ 
    ...
    my $priority = $net > 1 ? "(priority 100)" : "";
	print OUT <<EOF
...
EOF
  ...
  { 
	  ...
	  my $line = $_[3];
	  local * p = sub {
		my ($name, $off, $mm, $move, $neg) = @_;
		my $mil = bmil2(substr($value, $off, 4));
		assertdata("Via", $line, uc($name), $mil);
		if (defined $move) {
		  return sprintf("%.5f", $move + ($neg ? -1 : 1) * bmil2mm(substr($value, $off, 4)));
		}
		return sprintf("%.5f", bmil2mm(substr($value, $off, 4))) if $mm;
		return $mil;
	  };
	  ...
	  local * p = sub {
		...
		my $ret = unpack("C", substr($value, $off, 1));
		assertdata("Via", $line, uc($name), $ret);
		$ret;
	  };
	  
	  my ($CAGV, $CCSV, $CEN, $CENV, $UNIONINDEX) =
		(p("cagv", 40),
		 p("ccsv", 40),
		 p("cen", 23),
		 p("cenv", 40),
...
;
  ...
  { 
    ...
    foreach(0 .. 3)
    {
      ...
      $tpos+=2+$lengths[$_];
      $tpos+=1 if($_==1 && $lengths[1] == 0);
    }
    $linebreaks{$tpos}=1;

	print OUT "#Regions: starts: ", join(",", @starts), " lengths: ", join(",", @lengths), "\n" if $annotate;
	...
	local * p = sub {
	  ...
	  my $ret = unpack("s", substr($value, $off, 2));
	  assertdata("Region", $line, uc($name), $ret);
	  $ret;
	};

	my ($POLYGON, $component, $HOLECOUNT, $HOLE0VERTEXCOUNT) =
	  (p("polygon", 5),
	   p("component", 7),
	   p("holecount", 14),
	   p("hole0vertexcount", 14));
